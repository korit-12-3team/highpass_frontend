...
import { ArrowRight, Bookmark, Eye, Heart, MessageCircle, Trash2 } from "lucide-react";
        ...
        <div className="bg-white border-x border-b rounded-b-2xl overflow-hidden">
          ...
          <div className="px-4 pt-4 pb-3 border-b">
            <div className="flex gap-3 mb-3">
              ...
              <div>
                <p
                  ...
                  onClick={() => setProfileModal(viewPostModal.authorId)}
                >
                  {viewPostModal.author}
                </p>
                <p className="text-slate-400 text-xs">
                  @{viewPostModal.author.toLowerCase().replace(/\s/g, "_")}
                </p>
              </div>
            </div>
            {viewPostModal.title && <p className="font-bold text-base mb-1">{viewPostModal.title}</p>}
            <p className="text-slate-900 text-base leading-relaxed whitespace-pre-wrap mb-4">
              {viewPostModal.content}
            </p>
            <p className="text-slate-400 text-sm mb-4">
              {viewPostModal.createdAt || "오늘"} · <span className="font-bold text-slate-700">{viewPostModal.views}</span>{" "}
              조회
            </p>
            <div className="border-t border-b py-3 flex gap-6 text-slate-500 text-sm">
              <span>
                <span className="font-bold text-slate-900">{viewPostModal.comments?.length || 0}</span> 댓글
              </span>
              <span>
                <span className="font-bold text-slate-900">{viewPostModal.likes}</span> 좋아요
              </span>
              <span>
                <span className="font-bold text-slate-900">{viewPostModal.scraps || 0}</span> 북마크
              </span>
            </div>
            <div className="flex justify-around pt-1 text-slate-400">
              ...
              <button className="flex items-center gap-1.5 hover:text-green-500 p-2 rounded-full hover:bg-green-50 transition-colors text-sm">
                <Eye size={20} />
              </button>
            </div>
          </div>
          <div className="px-4 py-3 border-b flex gap-3 items-start">
            <div className="w-9 h-9 bg-hp-600 rounded-full flex items-center justify-center font-bold text-white shrink-0 text-sm">
            ...
            <div className="flex-1 flex gap-2">
              ...
              <button
                onClick={addComment}
                className="bg-hp-600 text-white text-xs font-bold px-4 py-1.5 rounded-full hover:bg-hp-700 transition-colors self-end"
              >
                등록
              </button>
            </div>
          ...
          <div className="divide-y divide-slate-100">
            {viewPostModal.comments?.length === 0 || !viewPostModal.comments ? (
              <div className="py-12 text-center text-slate-400 text-sm">아직 댓글이 없습니다.</div>
            ) : (
              viewPostModal.comments?.map(
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (comment: any) => (
                <div key={comment.id} className="px-4 py-3 flex gap-3 hover:bg-hp-50 transition-colors group">
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-slate-400 text-xs">· {comment.createdAt || "방금 전"}</span>
                      </div>
                    ...
                    <div className="flex gap-4 mt-2 text-slate-400">
                      <button className="hover:text-red-500 transition-colors flex items-center gap-1 text-xs">
                        <Heart size={14} />
                      </button>
                      <button className="hover:text-hp-500 transition-colors flex items-center gap-1 text-xs">
      ...
      <div className="bg-white border-x rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="border-b px-4 py-3 flex justify-end">
          <button
            ...
            className="bg-hp-600 text-white text-sm font-bold px-5 py-2 rounded-full hover:bg-hp-700 transition-colors"
          >
            글쓰기
          </button>
        </div>
        <div className="divide-y divide-slate-100">
          {boardData.filter((item) => item.type === "free").length === 0 ? (
            ...
            boardData
              ...
              .map((post) => (
                <div
                  key={post.id}
                  className="px-4 py-3 hover:bg-hp-50 cursor-pointer flex gap-3 transition-colors"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-5 text-slate-400 text-sm -ml-1">
                      <button
                        className="flex items-center gap-1.5 hover:text-hp-500 p-1 rounded-full hover:bg-hp-50 transition-colors"
                      ...
                      <button
                        className="flex items-center gap-1.5 hover:text-red-500 p-1 rounded-full hover:bg-red-50 transition-colors"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <Heart size={16} />
                      ...
                      >
                        <Bookmark size={16} />
                        <span className="text-xs">{post.scraps || 0}</span>
                      </button>

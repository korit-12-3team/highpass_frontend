...
import { useApp, type BoardPost, type SearchPlace } from "@/lib/AppContext";
...
import { deleteStudy, getStudy, updateStudy } from "@/lib/study-api";
...
export default function StudyPostPageClient() {
  ...
  const postId = useMemo(() => rawPostId || resolvePostViewRef("study", postRef), [postRef, rawPostId]);
  ...
  useEffect(() => {
    ...
    const ref = createPostViewRef("study", post.id);
    const nextUrl = `/study/post?ref=${encodeURIComponent(ref)}${returnTo ? `&returnTo=${encodeURIComponent(returnTo)}` : ""}`;
  ...
  const removePost = async () => {
    ...
    try {
      ...
      setBoardData((prev) => prev.filter((item) => !(item.type === "study" && item.id === post.id)));

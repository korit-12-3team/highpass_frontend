...
export async function listNotifications(userId: string): Promise<NotificationResponse[]> {
  const response = await fetchWithAuth(`${API_BASE_URL}/api/notifications/${userId}`);
  if (!response.ok) throw new Error("알림 목록을 불러오지 못했습니다.");
  return response.json();
}

export async function getUnreadCount(userId: string): Promise<number> {
  const response = await fetchWithAuth(`${API_BASE_URL}/api/notifications/unread-count/${userId}`);
...
export async function deleteAllNotifications(userId: string): Promise<void> {
  const response = await fetchWithAuth(`${API_BASE_URL}/api/notifications/all/${userId}`, {
    method: "DELETE",
  });

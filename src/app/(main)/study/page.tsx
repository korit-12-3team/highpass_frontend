...
import { Eye, Heart, MessageCircle, MapPin, X, ArrowRight, Trash2, Zap, Users } from 'lucide-react';
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-6 md:p-10">
          ...
          <div className="border-b pb-5 mb-5">
            ...
            <div className="flex justify-between items-center mt-5 text-sm font-medium text-slate-500">
              <div className="flex items-center gap-2 cursor-pointer hover:text-slate-800 transition-colors" onClick={() => setProfileModal(viewPostModal.authorId)}>
                <div className="w-8 h-8 bg-hp-100 rounded-full flex items-center justify-center text-hp-600 font-bold">{viewPostModal.author.substring(0, 1)}</div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <select value={certFilter} onChange={e => setCertFilter(e.target.value)} className="border rounded-xl px-3 py-2.5 text-sm outline-none focus:border-hp-500 appearance-none bg-white">
          ...
          {Object.entries(CERT_DATA).map(([cat, certs]) => (
            <optgroup key={cat} label={cat}>{certs.map(c => <option key={c} value={c}>{c}</option>)}</optgroup>
          ))}
        </select>
        ...
        {(certFilter || locationFilterSido) && (
          <button onClick={() => { setCertFilter(''); setLocationFilterSido(''); setLocationFilterSigungu(''); }} className="flex items-center gap-1 text-xs text-slate-500 border rounded-xl px-3 py-2 hover:bg-slate-50"><X size={12} /> 필터 초기화</button>
        )}
      </div>

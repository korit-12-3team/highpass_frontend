...
export default function ChatPage() {
  ...
  const sendMessage = () => {
    ...
    const msg = { id: Date.now(), senderId: currentUser.id, text: chatInput.trim(), createdAt: new Date().toTimeString().slice(0, 5) };
    setChatRooms(prev => prev.map(r => r.id === activeChatRoomId ? { ...r, messages: [...r.messages, msg] } : r));
    setChatInput('');
  };
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
              {chatRooms.map(room => (
                <button key={room.id} onClick={() => setActiveChatRoomId(room.id)}
                  ...
                  <div className="w-10 h-10 bg-hp-100 rounded-full flex items-center justify-center text-hp-600 font-bold text-sm shrink-0">{room.partnerNickname.substring(0, 1)}</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm text-slate-800 truncate">{room.partnerNickname}</p>
                    <p className="text-xs text-slate-400 truncate mt-0.5">{room.messages.length > 0 ? room.messages[room.messages.length - 1].text : '대화를 시작해보세요'}</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            return (
              <div className="flex-1 bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col">
                <div className="p-4 border-b flex items-center gap-3">
                  <div className="w-9 h-9 bg-hp-100 rounded-full flex items-center justify-center text-hp-600 font-bold text-sm">{room.partnerNickname.substring(0, 1)}</div>
                ...
                <div className="p-4 border-t flex gap-2">
                  <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') sendMessage(); }} placeholder="메시지를 입력하세요..." className="flex-1 border rounded-xl px-4 py-2.5 text-sm outline-none focus:border-hp-500" />
                  <button onClick={sendMessage} className="px-4 py-2.5 bg-hp-600 text-white rounded-xl font-bold hover:bg-hp-700 transition-colors"><ArrowRight size={18} /></button>
                </div>
              </div>
            );
          })() : (

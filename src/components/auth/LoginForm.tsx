...
export default function LoginForm() {
  ...
  const [kakaoReady, setKakaoReady] = useState(
    () => typeof window !== "undefined" && Boolean((window as KakaoWindow).Kakao?.Auth),
  );
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      const jsKey =
        process.env.NEXT_PUBLIC_KAKAO_JS_KEY_local || process.env.NEXT_PUBLIC_KAKAO_JS_KEY;
      if (kakao && jsKey && !kakao.isInitialized()) {
        kakao.init(jsKey);
      }
      setKakaoReady(true);
    };
  ...
  const handleLocalLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("서버에 연결할 수 없습니다. API 주소와 서버 상태를 확인해주세요.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthShell title="로그인" subtitle="계정에 로그인하세요">
      ...
      <div className="mt-6">
        <div className="relative flex items-center py-2 mb-4">
          ...
          <span className="flex-shrink-0 mx-4 text-hp-400 text-xs font-medium">또는 소셜 로그인</span>
          <div className="flex-grow border-t border-hp-200"></div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            disabled={kakaoLoading || !kakaoReady}
          ...
          >
            ...
            {kakaoLoading ? "이동 중..." : !kakaoReady ? "로딩 중..." : "카카오"}
          </button>
        </div>
      </div>
